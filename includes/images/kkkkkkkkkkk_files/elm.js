...
Elm.ElmForm = Elm.ElmForm || {};
Elm.ElmForm.Field = Elm.ElmForm.Field || {};
Elm.ElmForm.Field.make = function (_elm) {
   "use strict";
   _elm.ElmForm = _elm.ElmForm || {};
   _elm.ElmForm.Field = _elm.ElmForm.Field || {};
   if (_elm.ElmForm.Field.values) return _elm.ElmForm.Field.values;
   var _U = Elm.Native.Utils.make(_elm),
   $Basics = Elm.Basics.make(_elm),
   $Debug = Elm.Debug.make(_elm),
   $Dict = Elm.Dict.make(_elm),
   ...
   $String = Elm.String.make(_elm);
   var _op = {};
   var booleanChecked = function (value) {
      var _p0 = value;
      if (_p0.ctor === "Just") {
            return true;
         } else {
   ...
   var optionWithDefault = function (option) {
      return A2($Maybe.withDefault,option.label,option.value);
   };
   var blankOption = {label: ""
                     ,value: $Maybe.Just("")
                     ,checked: false};
   var classesForType = function (fieldType) {
      var _p1 = fieldType;
      if (_p1 === "date") {
            return " date-picker";
         } else {
   ...
   var schemaValue = function (_p2) {
      var _p3 = _p2;
      return _p3._0;
   };
   var schemaType = function (schema) {
      return function (_) {
         return _.type$;
      }(schemaValue(schema));
   };
   var schemaId = function (schema) {
      return function (_) {
         return _.id;
      }(schemaValue(schema));
   };
   var schemaName = function (model) {
      var _p4 = model.parentIds;
      if (_p4.ctor === "Nothing") {
            return schemaId(model.schema);
         } else {
            return A2($Basics._op["++"],
            A2($String.join,".",_p4._0),
            A2($Basics._op["++"],
            ".",
            A2($Basics._op["++"],schemaId(model.schema),"[]")));
         }
   };
   var viewBoolean = F2(function (address,model) {
      return A2($Html.input,
      _U.list([$Html$Attributes.type$("checkbox")
              ,$Html$Attributes.name(schemaName(model))
              ,$Html$Attributes.value("yes")
              ,$Html$Attributes.checked(booleanChecked(model.value))]),
      _U.list([]));
   });
   var viewFile = F2(function (address,model) {
      return A2($Html.input,
      _U.list([$Html$Attributes.type$("file")
              ,$Html$Attributes.name(schemaName(model))]),
      _U.list([]));
   });
   var schemaTitle = function (schema) {
      return function (_) {
         return _.title;
      }(schemaValue(schema));
   };
   var schemaFieldOptions = function (schema) {
      return function (_) {
         return _.field_options;
      }(schemaValue(schema));
   };
   var schemaRequired = function (schema) {
      return function (_) {
         return _.required;
      }(schemaValue(schema));
   };
   var listValueToList = function (maybeValue) {
      var _p5 = maybeValue;
      if (_p5.ctor === "Just") {
            var _p6 = _p5._0;
            if (_p6.ctor === "ListValue") {
                  return _p6._0;
               } else {
                  return _U.crashCase("ElmForm.Field",
                  {start: {line: 157,column: 7},end: {line: 161,column: 60}},
                  _p6)("Cant convert non-list value to list");
               }
         } else {
            return _U.list([]);
         }
   };
   var scalarValueToString = function (maybeValue) {
      var _p8 = maybeValue;
      if (_p8.ctor === "Just") {
            var _p9 = _p8._0;
            switch (_p9.ctor)
            {case "StringValue": return _p9._0;
               case "IntValue": return $Basics.toString(_p9._0);
               default: return _U.crashCase("ElmForm.Field",
                 {start: {line: 139,column: 7},end: {line: 145,column: 63}},
                 _p9)("Cant convert non scalar value to strig");}
         } else {
            return "";
         }
   };
   var optionChecked = F3(function (fieldType,option,model) {
      var _p11 = fieldType;
      switch (_p11)
      {case "radio": return _U.eq(scalarValueToString(model.value),
           optionWithDefault(option)) ? true : false;
         case "checkbox": return A2($List.member,
           optionWithDefault(option),
           listValueToList(model.value));
         default: return _U.crashCase("ElmForm.Field",
           {start: {line: 281,column: 3},end: {line: 288,column: 59}},
           _p11)(A2($Basics._op["++"],
           fieldType,
           " is not an option type"));}
   });
   var viewDropdownOption = F2(function (fieldOption,model) {
      return A2($Html.option,
      _U.list([$Html$Attributes.selected(_U.eq(optionWithDefault(fieldOption),
      scalarValueToString(model.value)) ? true : false)]),
      _U.list([$Html.text(fieldOption.label)]));
   });
   var viewErrors = function (maybeErrors) {
      var _p13 = maybeErrors;
      if (_p13.ctor === "Nothing") {
            ...
            function (error) {
               ...
               _U.list([$Html.text(error)]));
            },
            _p13._0);
         }
   };
   var view = F3(function (viewFunc,address,model) {
      return A2($Html.div,
      _U.list([$Html$Attributes.$class("col-md-6")]),
      A2($Basics._op["++"],
      _U.list([A2(viewFunc,address,model)]),
      viewErrors(model.errors)));
   });
   var update = F2(function (action,model) {
      var _p14 = action;
      return _U.update(model,{value: _p14._0});
   });
   var init = F5(function (uniqueId,
   schema,
   value,
   errors,
   parentIds) {
   ...
   var NestedValue = function (a) {
      return {ctor: "NestedValue",_0: a};
   };
   ...
   var viewText = F2(function (address,model) {
      return A2($Html.input,
      _U.list([$Html$Attributes.type$("text")
              ,$Html$Attributes.id(model.uniqueId)
              ,$Html$Attributes.name(schemaName(model))
              ,$Html$Attributes.value(scalarValueToString(model.value))
              ,$Html$Attributes.$class(A2($Basics._op["++"],
              "form-control",
              classesForType(schemaType(model.schema))))
              ,A3($Html$Events.on,
              "change",
              ...
              function (value) {
                 ...
                 address,
                 Change($Maybe.Just(StringValue(value))));
              })]),
      _U.list([]));
   });
   var optionChange = F3(function (address,fieldType,value) {
      var _p15 = fieldType;
      switch (_p15)
      {case "radio": return A2($Signal.message,
           address,
           Change($Maybe.Just(StringValue(value))));
         case "checkbox": return A2($Signal.message,
           address,
           Change($Maybe.Just(ListValue(_U.list([value])))));
         default: return _U.crashCase("ElmForm.Field",
           {start: {line: 263,column: 3},end: {line: 270,column: 65}},
           _p15)("Can only use this with radios and checkboxes");}
   });
   var viewOption = F4(function (fieldType,address,option,model) {
      return A2($Html.div,
      _U.list([$Html$Attributes.$class(fieldType)]),
      ...
      _U.list([A2($Html.input,
              _U.list([$Html$Attributes.type$(fieldType)
                      ...
                      schemaName(model),
                      _U.eq(schemaType(model.schema),"checkboxes") ? "[]" : ""))
                      ,$Html$Attributes.value(optionWithDefault(option))
                      ,$Html$Attributes.checked(A3(optionChecked,
                      fieldType,
                      option,
                      model))
                      ,A3($Html$Events.on,
                      ...
                      $Html$Events.targetValue,
                      function (value) {
                         return A3(optionChange,address,fieldType,value);
                      })]),
              _U.list([]))
              ,$Html.text(option.label)]))]));
   });
   var viewOptions = F3(function (fieldType,address,model) {
      return A2($Html.div,
      _U.list([]),
      A2($List.map,
      function (option) {
         return A4(viewOption,fieldType,address,option,model);
      },
      A2($Maybe.withDefault,
      _U.list([]),
      function (_) {
         return _.options;
      }(schemaFieldOptions(model.schema)))));
   });
   var viewParagraph = F2(function (address,model) {
      return A2($Html.textarea,
      _U.list([$Html$Attributes.name(schemaName(model))
      ...
      _U.list([$Html.text(scalarValueToString(model.value))]));
   });
   var viewDropdown = F2(function (address,model) {
      ...
      A2($Basics._op["++"],
      _U.list([A2(viewDropdownOption,blankOption,model)]),
      A2($List.map,
      function (option) {
         return A2(viewDropdownOption,option,model);
      },
      A2($Maybe.withDefault,
      _U.list([]),
      function (_) {
   ...
   var Schema = function (a) {    return {ctor: "Schema",_0: a};};
   var initSchema = F5(function (type$,
   id,
   title,
   fieldOptions,
   required) {
   ...
   var FieldDef = F5(function (a,b,c,d,e) {
      return {type$: a
             ,id: b
             ,title: c
             ,field_options: d
   ...
   var FieldOptions = F4(function (a,b,c,d) {
      return {options: a
             ,size: b
             ,include_blank_option: c
             ,fields: d};
   });
   var Option = F3(function (a,b,c) {
      return {label: a,checked: b,value: c};
   });
   return _elm.ElmForm.Field.values = {_op: _op
...
Elm.ElmForm = Elm.ElmForm || {};
Elm.ElmForm.FieldSet = Elm.ElmForm.FieldSet || {};
Elm.ElmForm.FieldSet.make = function (_elm) {
   "use strict";
   _elm.ElmForm = _elm.ElmForm || {};
   _elm.ElmForm.FieldSet = _elm.ElmForm.FieldSet || {};
   if (_elm.ElmForm.FieldSet.values)
   return _elm.ElmForm.FieldSet.values;
   var _U = Elm.Native.Utils.make(_elm),
   $Array = Elm.Array.make(_elm),
   $Basics = Elm.Basics.make(_elm),
   $Debug = Elm.Debug.make(_elm),
   $Dict = Elm.Dict.make(_elm),
   $ElmForm$Field = Elm.ElmForm.Field.make(_elm),
   $Html = Elm.Html.make(_elm),
   $Html$Attributes = Elm.Html.Attributes.make(_elm),
   $Html$Events = Elm.Html.Events.make(_elm),
   $List = Elm.List.make(_elm),
   ...
   $String = Elm.String.make(_elm);
   var _op = {};
   var generateFieldId = F3(function (parentIds,index,fieldId) {
      var _p0 = parentIds;
      if (_p0.ctor === "Nothing") {
            return fieldId;
         } else {
            return A2($Basics._op["++"],
            A2($String.join,".",_p0._0),
            A2($Basics._op["++"],
            ".",
            A2($Basics._op["++"],
            $Basics.toString(index),
            A2($Basics._op["++"],".",fieldId))));
         }
   });
   var viewFromType = function (fieldType) {
      var _p1 = fieldType;
      switch (_p1)
      {case "text": return $ElmForm$Field.viewText;
         case "number": return $ElmForm$Field.viewText;
         case "date": return $ElmForm$Field.viewText;
         case "radio": return $ElmForm$Field.viewOptions("radio");
         case "checkboxes":
         return $ElmForm$Field.viewOptions("checkbox");
         case "boolean": return $ElmForm$Field.viewBoolean;
         case "paragraph": return $ElmForm$Field.viewParagraph;
         case "dropdown": return $ElmForm$Field.viewDropdown;
         case "file": return $ElmForm$Field.viewFile;
         default: return $ElmForm$Field.viewText;}
   };
   var requiredClass = function (fieldNode) {
      var _p2 = fieldNode;
      if (_p2.ctor === "Node") {
            return $ElmForm$Field.schemaRequired(_p2._0.schema) ? " required" : "";
         } else {
            return "";
         }
   };
   var errorClass = function (fieldNode) {
      var _p3 = fieldNode;
      if (_p3.ctor === "Node") {
            return _U.eq(_p3._0.errors,
            $Maybe.Nothing) ? "" : " has-error";
         } else {
            return "";
         }
   };
   var fieldFromFieldNode = function (fieldNode) {
      var _p4 = fieldNode;
      if (_p4.ctor === "Node") {
            return _p4._0;
         } else {
   ...
   var removeItemFromList = F2(function (index,list) {
      var array = $Array.fromList(list);
      var arrayLength = $Array.length(array);
      var list1 = A3($Array.slice,0,A2($Basics.max,0,index),array);
      var list2 = A3($Array.slice,
      A2($Basics.min,arrayLength,index + 1),
      arrayLength,
      array);
      return A2($Basics._op["++"],
      $Array.toList(list1),
   ...
   var childParentIds = function (field) {
      var _p5 = field.parentIds;
      if (_p5.ctor === "Nothing") {
            return $Maybe.Just(_U.list([field.uniqueId]));
         } else {
            return $Maybe.Just(A2($Basics._op["++"],
            _p5._0,
            _U.list([field.uniqueId])));
         }
   };
   var init = function (fields) {    return fields;};
   var empty = _U.list([]);
   var ErrorTree = function (a) {
      return {ctor: "ErrorTree",_0: a};
   };
   var ErrorNode = function (a) {
      return {ctor: "ErrorNode",_0: a};
   };
   var FieldTree = F3(function (a,b,c) {
      return {ctor: "FieldTree",_0: a,_1: b,_2: c};
   });
   var appendChildNodes = F3(function (fieldNode,
   uniqueFieldId,
   childFieldNodes) {
      var uniqueId = function () {
         var _p6 = fieldNode;
         if (_p6.ctor === "Node") {
               return $ElmForm$Field.schemaName(_p6._0);
            } else {
               return $ElmForm$Field.schemaName(_p6._1);
            }
      }();
      if (_U.eq(uniqueId,uniqueFieldId)) {
            var _p7 = fieldNode;
            if (_p7.ctor === "FieldTree") {
                  return A3(FieldTree,
                  _p7._0 + 1,
                  _p7._1,
                  A2($Basics._op["++"],_p7._2,_U.list([childFieldNodes])));
               } else {
                  return _U.crashCase("ElmForm.FieldSet",
                  {start: {line: 136,column: 7},end: {line: 140,column: 44}},
                  _p7)("Cant append to Node");
               }
         } else return fieldNode;
   });
   var removeChildNodesAtIndex = F3(function (fieldNode,
   targetUniqueId,
   index) {
      var uniqueId = function () {
         var _p9 = fieldNode;
         if (_p9.ctor === "Node") {
               return $ElmForm$Field.schemaName(_p9._0);
            } else {
               return $ElmForm$Field.schemaName(_p9._1);
            }
      }();
      if (_U.eq(uniqueId,targetUniqueId)) {
            var _p10 = fieldNode;
            if (_p10.ctor === "FieldTree") {
                  return A3(FieldTree,
                  _p10._0,
                  _p10._1,
                  A2(removeItemFromList,index,_p10._2));
               } else {
                  return _U.crashCase("ElmForm.FieldSet",
                  {start: {line: 154,column: 7},end: {line: 158,column: 46}},
                  _p10)("Cant remove from Node");
               }
         } else return fieldNode;
   });
   var Node = function (a) {    return {ctor: "Node",_0: a};};
   var updateMatchingField = F3(function (uniqueId,action,node) {
      var _p12 = node;
      if (_p12.ctor === "Node") {
            var _p13 = _p12._0;
            return _U.eq(_p13.uniqueId,
            uniqueId) ? Node(A2($ElmForm$Field.update,
            action,
            _p13)) : Node(_p13);
         } else {
            return A3(FieldTree,
            ...
            function (childGroup) {
               return A2($List.map,
               function (childNode) {
                  return A3(updateMatchingField,uniqueId,action,childNode);
               },
               childGroup);
            },
            _p12._2));
   ...
   var createEmptyFieldNode = F3(function (currentIndex,
   parentField,
   _p14) {
      var _p15 = _p14;
      var _p16 = _p15._0;
      var parentIds = childParentIds(parentField);
      return Node(A5($ElmForm$Field.init,
      A3(generateFieldId,
      parentIds,
      currentIndex,
      $ElmForm$Field.schemaId($ElmForm$Field.Schema(_p16))),
      $ElmForm$Field.Schema(_p16),
      $Maybe.Nothing,
      $Maybe.Nothing,
      parentIds));
   });
   var createChildFieldNodes = F3(function (currentIndex,
   parentField,
   maybeFields) {
      var _p17 = maybeFields;
      if (_p17.ctor === "Just") {
            return A2($List.map,
            A2(createEmptyFieldNode,currentIndex,parentField),
            _p17._0);
         } else {
            return _U.list([]);
         }
   ...
   var update = F2(function (action,model) {
      ...
      {case "NoOp": return model;
         case "Update": return A2($List.map,
           function (node) {
              return A3(updateMatchingField,_p18._0,_p18._1,node);
           },
           model);
         case "AddFieldset": var _p19 = _p18._1;
           var uniqueFieldId = $ElmForm$Field.schemaName(_p19);
           var childFieldNodes = A3(createChildFieldNodes,
           _p18._0,
           _p19,
           function (_) {
              return _.fields;
           }($ElmForm$Field.schemaFieldOptions(_p19.schema)));
           return A2($List.map,
           function (node) {
              return A3(appendChildNodes,
              node,
              uniqueFieldId,
              childFieldNodes);
           },
           model);
         default: var uniqueFieldId = $ElmForm$Field.schemaName(_p18._1);
           return A2($List.map,
           function (node) {
              return A3(removeChildNodesAtIndex,
              node,
              uniqueFieldId,
              _p18._0);
           },
   ...
   var RemoveFieldset = F2(function (a,b) {
      return {ctor: "RemoveFieldset",_0: a,_1: b};
   });
   var viewRemoveChild = F3(function (address,index,parentField) {
      return A2($Html.div,
      _U.list([$Html$Attributes.$class("col-md-offset-2")]),
      _U.list([A2($Html.a,
      _U.list([$Html$Attributes.href("javascript:;")
              ,A2($Html$Events.onClick,
              address,
              A2(RemoveFieldset,index,parentField))]),
      _U.list([$Html.text("Remove")]))]));
   });
   var AddFieldset = F2(function (a,b) {
      return {ctor: "AddFieldset",_0: a,_1: b};
   });
   var viewAddFieldset = F3(function (address,currentIndex,field) {
      return _U.list([A2($Html.div,
      _U.list([$Html$Attributes.$class("col-md-offset-1")]),
      _U.list([A2($Html.a,
      _U.list([$Html$Attributes.href("javascript:;")
              ,A2($Html$Events.onClick,
              address,
              A2(AddFieldset,currentIndex,field))]),
      _U.list([$Html.text("Add New")]))]))]);
   });
   var Update = F2(function (a,b) {
      return {ctor: "Update",_0: a,_1: b};
   });
   var viewField = F2(function (address,field) {
      return A3($ElmForm$Field.view,
      viewFromType($ElmForm$Field.schemaType(field.schema)),
      A2($Signal.forwardTo,address,Update(field.uniqueId)),
      field);
   });
   var viewFieldNode = F2(function (address,fieldNode) {
      var _p20 = fieldNode;
      if (_p20.ctor === "Node") {
            return A2(viewField,address,_p20._0);
         } else {
            var _p21 = _p20._1;
            ...
            A2($Basics._op["++"],
            A2($List.indexedMap,
            F2(function (index,child) {
               return A4(viewFieldSetChild,address,index,_p21,child);
            }),
            _p20._2),
            A3(viewAddFieldset,address,_p20._0,_p21)));
         }
   });
   var viewFieldSetChild = F4(function (address,
   index,
   parentField,
   child) {
      return A2($Html.div,
      _U.list([]),
      _U.list([A2(view,address,child)
              ,A3(viewRemoveChild,address,index,parentField)]));
   });
   var view = F2(function (address,model) {
      ...
      function (fieldNode) {
         ...
         "form-group",
         errorClass(fieldNode)))]),
         _U.list([A2($Html.label,
                 _U.list([$Html$Attributes.$class(A2($Basics._op["++"],
                         "col-md-2 control-label",
                         requiredClass(fieldNode)))
                         ,$Html$Attributes.$for(function (_) {
                            return _.uniqueId;
                         }(fieldFromFieldNode(fieldNode)))]),
                 _U.list([$Html.text($ElmForm$Field.schemaTitle(function (_) {
                    return _.schema;
                 }(fieldFromFieldNode(fieldNode))))]))
                 ,A2(viewFieldNode,address,fieldNode)]));
      },
      model));
   });
   var NoOp = {ctor: "NoOp"};
   return _elm.ElmForm.FieldSet.values = {_op: _op
                                         ,empty: empty
                                         ,init: init
                                         ,update: update
                                         ,view: view
...
Elm.ElmForm = Elm.ElmForm || {};
Elm.ElmForm.JsonDecoder = Elm.ElmForm.JsonDecoder || {};
Elm.ElmForm.JsonDecoder.make = function (_elm) {
   "use strict";
   _elm.ElmForm = _elm.ElmForm || {};
   _elm.ElmForm.JsonDecoder = _elm.ElmForm.JsonDecoder || {};
   if (_elm.ElmForm.JsonDecoder.values)
   return _elm.ElmForm.JsonDecoder.values;
   var _U = Elm.Native.Utils.make(_elm),
   $Basics = Elm.Basics.make(_elm),
   $Debug = Elm.Debug.make(_elm),
   $ElmForm$Field = Elm.ElmForm.Field.make(_elm),
   $ElmForm$FieldSet = Elm.ElmForm.FieldSet.make(_elm),
   $Json$Decode = Elm.Json.Decode.make(_elm),
   $List = Elm.List.make(_elm),
   $Maybe = Elm.Maybe.make(_elm),
   $Result = Elm.Result.make(_elm),
   $Signal = Elm.Signal.make(_elm);
   var _op = {};
   var decodeNestedErrors = $Json$Decode.dict(A2($Json$Decode.map,
   $ElmForm$FieldSet.ErrorNode,
   $Json$Decode.list($Json$Decode.string)));
   var decodeErrors = $Json$Decode.dict($Json$Decode.oneOf(_U.list([A2($Json$Decode.map,
                                                                   $ElmForm$FieldSet.ErrorNode,
                                                                   $Json$Decode.list($Json$Decode.string))
                                                                   ,A2($Json$Decode.map,
                                                                   $ElmForm$FieldSet.ErrorTree,
                                                                   $Json$Decode.list(decodeNestedErrors))])));
   var decodeErrorsPayload = function (errorsPayload) {
      return A2($Json$Decode.decodeString,
      decodeErrors,
      errorsPayload);
   };
   var decodeNestedValue = $Json$Decode.dict($Json$Decode.oneOf(_U.list([A2($Json$Decode.map,
                                                                        $ElmForm$Field.StringValue,
                                                                        $Json$Decode.string)
                                                                        ,A2($Json$Decode.map,$ElmForm$Field.IntValue,$Json$Decode.$int)
                                                                        ,A2($Json$Decode.map,$ElmForm$Field.BoolValue,$Json$Decode.bool)
                                                                        ,A2($Json$Decode.map,
                                                                        $ElmForm$Field.ListValue,
                                                                        $Json$Decode.list($Json$Decode.string))
                                                                        ,$Json$Decode.$null($ElmForm$Field.StringValue(""))])));
   var decodeValue = $Json$Decode.oneOf(_U.list([A2($Json$Decode.map,
                                                $ElmForm$Field.StringValue,
                                                $Json$Decode.string)
                                                ,A2($Json$Decode.map,$ElmForm$Field.IntValue,$Json$Decode.$int)
                                                ,A2($Json$Decode.map,$ElmForm$Field.BoolValue,$Json$Decode.bool)
                                                ...
                                                $Json$Decode.list($Json$Decode.string))
                                                ,A2($Json$Decode.map,
                                                $ElmForm$Field.NestedValue,
                                                $Json$Decode.list(decodeNestedValue))
                                                ,$Json$Decode.$null($ElmForm$Field.StringValue(""))]));
   var decodeValuesPayload = function (valuePayload) {
      return A2($Json$Decode.decodeString,
      $Json$Decode.dict(decodeValue),
      valuePayload);
   };
   var decodeOptions = A4($Json$Decode.object3,
   $ElmForm$Field.Option,
   A2($Json$Decode._op[":="],"label",$Json$Decode.string),
   A2($Json$Decode._op[":="],"checked",$Json$Decode.bool),
   $Json$Decode.maybe(A2($Json$Decode._op[":="],
   "value",
   $Json$Decode.string)));
   var initChildFieldOptions = F3(function (options,
   size,
   include_blank_option) {
      return A4($ElmForm$Field.FieldOptions,
      options,
      size,
      include_blank_option,
      $Maybe.Nothing);
   });
   var decodeNestedFieldOptions = A4($Json$Decode.object3,
   initChildFieldOptions,
   $Json$Decode.maybe(A2($Json$Decode._op[":="],
   "options",
   $Json$Decode.list(decodeOptions))),
   $Json$Decode.maybe(A2($Json$Decode._op[":="],
   "size",
   $Json$Decode.string)),
   $Json$Decode.maybe(A2($Json$Decode._op[":="],
   "include_blank_option",
   $Json$Decode.bool)));
   var decodeNestedField = A6($Json$Decode.object5,
   $ElmForm$Field.initSchema,
   ...
   A2($Json$Decode._op[":="],
   "field_options",
   decodeNestedFieldOptions),
   A2($Json$Decode._op[":="],"required",$Json$Decode.bool));
   var decodeFieldOptions = A5($Json$Decode.object4,
   $ElmForm$Field.FieldOptions,
   $Json$Decode.maybe(A2($Json$Decode._op[":="],
   "options",
   $Json$Decode.list(decodeOptions))),
   $Json$Decode.maybe(A2($Json$Decode._op[":="],
   ...
   $Json$Decode.maybe(A2($Json$Decode._op[":="],
   "fields",
   $Json$Decode.list(decodeNestedField))));
   var decodeField = A6($Json$Decode.object5,
   $ElmForm$Field.initSchema,
   A2($Json$Decode._op[":="],"type",$Json$Decode.string),
   A2($Json$Decode._op[":="],"id",$Json$Decode.string),
   A2($Json$Decode._op[":="],"title",$Json$Decode.string),
   A2($Json$Decode._op[":="],"field_options",decodeFieldOptions),
   A2($Json$Decode._op[":="],"required",$Json$Decode.bool));
   var decodeFieldsPayload = function (fieldPayload) {
      return A2($Json$Decode.decodeString,
   ...
   return _elm.ElmForm.JsonDecoder.values = {_op: _op
                                            ,decodeFieldsPayload: decodeFieldsPayload
                                            ,decodeField: decodeField
                                            ,decodeFieldOptions: decodeFieldOptions
                                            ,decodeOptions: decodeOptions
                                            ,decodeValuesPayload: decodeValuesPayload
                                            ,decodeNestedValue: decodeNestedValue
                                            ,decodeErrorsPayload: decodeErrorsPayload};
};
Elm.ElmForm = Elm.ElmForm || {};
Elm.ElmForm.make = function (_elm) {
   "use strict";
   _elm.ElmForm = _elm.ElmForm || {};
   if (_elm.ElmForm.values) return _elm.ElmForm.values;
   var _U = Elm.Native.Utils.make(_elm),
   $Array = Elm.Array.make(_elm),
   $Basics = Elm.Basics.make(_elm),
   $Debug = Elm.Debug.make(_elm),
   ...
   $ElmForm$Field = Elm.ElmForm.Field.make(_elm),
   $ElmForm$FieldSet = Elm.ElmForm.FieldSet.make(_elm),
   $ElmForm$JsonDecoder = Elm.ElmForm.JsonDecoder.make(_elm),
   $List = Elm.List.make(_elm),
   $Maybe = Elm.Maybe.make(_elm),
   $Result = Elm.Result.make(_elm),
   $Signal = Elm.Signal.make(_elm);
   var _op = {};
   var getChildValues = function (values) {
      var _p0 = values;
      if (_p0.ctor === "NestedValue") {
            return _p0._0;
         } else {
            return _U.list([]);
         }
   };
   var getChildParentIds = F2(function (parentIds,schemaId) {
      var _p1 = parentIds;
      if (_p1.ctor === "Nothing") {
            return $Maybe.Just(_U.list([schemaId]));
         } else {
            return $Maybe.Just(A2($Basics._op["++"],
            _p1._0,
            _U.list([schemaId])));
   ...
   var getChildSchemas = function (childFields) {
      var _p2 = childFields;
      if (_p2.ctor === "Nothing") {
            return _U.list([]);
         } else {
   ...
   var errorsFromErrorNode = function (maybeErrorNode) {
      var _p3 = maybeErrorNode;
      if (_p3.ctor === "Nothing") {
            return $Maybe.Nothing;
         } else {
            if (_p3._0.ctor === "ErrorNode") {
                  return $Maybe.Just(_p3._0._0);
               } else {
                  return _U.crashCase("ElmForm",
                  {start: {line: 114,column: 3},end: {line: 120,column: 31}},
                  _p3)("Unexpected");
               }
         }
   };
   var errorArrayFromErrorNode = function (maybeErrorNode) {
      var _p5 = maybeErrorNode;
      if (_p5.ctor === "Nothing") {
            return $Array.empty;
         } else {
            if (_p5._0.ctor === "ErrorTree") {
                  return $Array.fromList(_p5._0._0);
               } else {
                  return _U.crashCase("ElmForm",
                  {start: {line: 102,column: 3},end: {line: 108,column: 31}},
                  _p5)("Unexpected");
               }
         }
   };
   var createFieldSet = F5(function (index,
   schemas,
   values,
   errors,
   parentIds) {
      ...
      function (_p7) {
         var _p8 = _p7;
         var _p9 = _p8._0;
         return A5(createFieldNode,
         index,
         A2($Dict.get,_p9.id,values),
         A2($Dict.get,_p9.id,errors),
         parentIds,
         _p9);
      },
      schemas);
   });
   var createFieldNode = F5(function (index,
   value,
   errors,
   parentIds,
   field) {
      var _p10 = field.type$;
      if (_p10 === "fieldset") {
            var errorArray = $Array.fromList;
            var childParentIds = A2(getChildParentIds,parentIds,field.id);
            var childSchemas = getChildSchemas(field.field_options.fields);
            var childValues = getChildValues(A2($Maybe.withDefault,
            $ElmForm$Field.NestedValue(_U.list([])),
            value));
            var currentIndex = $List.length(childValues);
            return A3($ElmForm$FieldSet.FieldTree,
            currentIndex,
            A5($ElmForm$Field.init,
            A3($ElmForm$FieldSet.generateFieldId,parentIds,index,field.id),
            $ElmForm$Field.Schema(field),
            value,
            $Maybe.Nothing,
            parentIds),
            A2($List.indexedMap,
            F2(function (childIndex,values) {
               return A5(createFieldSet,
               childIndex,
               childSchemas,
               values,
               A2($Maybe.withDefault,
               $Dict.empty,
               A2($Array.get,childIndex,errorArrayFromErrorNode(errors))),
               childParentIds);
            }),
            childValues));
         } else {
            return $ElmForm$FieldSet.Node(A5($ElmForm$Field.init,
            A3($ElmForm$FieldSet.generateFieldId,parentIds,index,field.id),
            $ElmForm$Field.Schema(field),
            value,
            errorsFromErrorNode(errors),
            parentIds));
         }
   });
   var okOrCrash = function (result) {
      var _p11 = result;
      if (_p11.ctor === "Ok") {
            return _p11._0;
         } else {
            return _U.crashCase("ElmForm",
            {start: {line: 17,column: 3},end: {line: 21,column: 26}},
            _p11)(_p11._0);
         }
   };
   var processFieldsPayload = function (fieldPayload) {
      return okOrCrash($ElmForm$JsonDecoder.decodeFieldsPayload(fieldPayload));
   };
   var processValuesPayload = function (valuesPayload) {
      return okOrCrash($ElmForm$JsonDecoder.decodeValuesPayload(valuesPayload));
   };
   var processErrorsPayload = function (errorsPayload) {
      return okOrCrash($ElmForm$JsonDecoder.decodeErrorsPayload(errorsPayload));
   };
   var processPayload = F3(function (fieldsPayload,
   valuesPayload,
   errorsPayload) {
      return A5(createFieldSet,
      0,
      processFieldsPayload(fieldsPayload),
      processValuesPayload(valuesPayload),
      processErrorsPayload(errorsPayload),
      $Maybe.Nothing);
   });
   return _elm.ElmForm.values = {_op: _op
                                ,processPayload: processPayload
                                ,createFieldSet: createFieldSet};
};
Elm.ElmForm = Elm.ElmForm || {};
Elm.ElmForm.JsonEncoder = Elm.ElmForm.JsonEncoder || {};
Elm.ElmForm.JsonEncoder.make = function (_elm) {
   "use strict";
   _elm.ElmForm = _elm.ElmForm || {};
   _elm.ElmForm.JsonEncoder = _elm.ElmForm.JsonEncoder || {};
   if (_elm.ElmForm.JsonEncoder.values)
   return _elm.ElmForm.JsonEncoder.values;
   var _U = Elm.Native.Utils.make(_elm),
   $Basics = Elm.Basics.make(_elm),
   $Debug = Elm.Debug.make(_elm),
   $ElmForm$Field = Elm.ElmForm.Field.make(_elm),
   ...
   $Signal = Elm.Signal.make(_elm);
   var _op = {};
   var encodeValue = function (field) {
      var _p0 = field.value;
      if (_p0.ctor === "Just") {
            var _p1 = _p0._0;
            switch (_p1.ctor)
            {case "StringValue": return $Json$Encode.string(_p1._0);
               case "IntValue": return $Json$Encode.$int(_p1._0);
               case "BoolValue": return $Json$Encode.bool(_p1._0);
               case "ListValue": return $Json$Encode.list(A2($List.map,
                 function (v) {
                    return $Json$Encode.string(v);
                 },
                 _p1._0));
   ...
   var encodeFieldNode = function (node) {
      var _p2 = node;
      if (_p2.ctor === "Node") {
            var _p3 = _p2._0;
            ...
            $ElmForm$Field.schemaId(_p3.schema),
            encodeValue(_p3));
         } else {
            return A2(F2(function (v0,v1) {
               return {ctor: "_Tuple2",_0: v0,_1: v1};
            }),
            $ElmForm$Field.schemaId(_p2._1.schema),
   ...
   var encodeFieldSet = function (fieldNodes) {
      return A2($Json$Encode.encode,
      0,
      $Json$Encode.object(A2($List.map,encodeFieldNode,fieldNodes)));
   };
   return _elm.ElmForm.JsonEncoder.values = {_op: _op
                                            ,encodeFieldSet: encodeFieldSet};
};
Elm.ElmForm = Elm.ElmForm || {};
Elm.ElmForm.Examples = Elm.ElmForm.Examples || {};
Elm.ElmForm.Examples.Modal = Elm.ElmForm.Examples.Modal || {};
Elm.ElmForm.Examples.Modal.make = function (_elm) {
   "use strict";
   _elm.ElmForm = _elm.ElmForm || {};
   _elm.ElmForm.Examples = _elm.ElmForm.Examples || {};
   _elm.ElmForm.Examples.Modal = _elm.ElmForm.Examples.Modal || {};
   if (_elm.ElmForm.Examples.Modal.values)
   return _elm.ElmForm.Examples.Modal.values;
   var _U = Elm.Native.Utils.make(_elm),
   $Basics = Elm.Basics.make(_elm),
   $Debug = Elm.Debug.make(_elm),
   $ElmForm = Elm.ElmForm.make(_elm),
   $ElmForm$FieldSet = Elm.ElmForm.FieldSet.make(_elm),
   $ElmForm$JsonEncoder = Elm.ElmForm.JsonEncoder.make(_elm),
   $Html = Elm.Html.make(_elm),
   $List = Elm.List.make(_elm),
   $Maybe = Elm.Maybe.make(_elm),
   $Result = Elm.Result.make(_elm),
   ...
   $Task = Elm.Task.make(_elm);
   var _op = {};
   var load = function (_p0) {
      var _p1 = _p0;
      return $ElmForm$FieldSet.init(A3($ElmForm.processPayload,
      _p1._0,
      _p1._1,
      _p1._2));
   };
   var payload = Elm.Native.Port.make(_elm).inboundSignal("payload",
   ...
   var update = F2(function (action,model) {
      ...
      switch (_p2.ctor)
      {case "NoOp": return model;
         case "Load": return _p2._0;
         default: return A2($ElmForm$FieldSet.update,_p2._0,model);}
   });
   var UpdateFieldSet = function (a) {
      return {ctor: "UpdateFieldSet",_0: a};
   };
   var view = F2(function (address,model) {
      return A2($ElmForm$FieldSet.view,
      A2($Signal.forwardTo,address,UpdateFieldSet),
      model);
   });
   ...
   update,
   $ElmForm$FieldSet.empty,
   actions.signal);
   var main = A2($Signal.map,view(actions.address),model);
   var saveValues = Elm.Native.Port.make(_elm).outboundSignal("saveValues",
   function (v) {
      return v;
   },
   A2($Signal.map,
   function (m) {
      return $ElmForm$JsonEncoder.encodeFieldSet(m);
   },
   model));
   var payloadMapper = Elm.Native.Task.make(_elm).performSignal("payloadMapper",
   A2($Signal.map,
   function (data) {
      return A2($Signal.send,actions.address,Load(load(data)));
   },
   payload));
   return _elm.ElmForm.Examples.Modal.values = {_op: _op
                                               ,NoOp: NoOp
                                               ,Load: Load
